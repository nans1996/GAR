...
public class UserRoleController implements Serializable {
    ...
    @EJB
    private dao.UserRoleFacade ejbFacade;
    ...
    public UserRole getSelected() {
        if (current == null) {
            ...
            current.setUserRolePK(new entity.UserRolePK());
            selectedItemIndex = -1;
        }
        return current;
    }
    ...
    public String create() {
        try {
            current.getUserRolePK().setLogin(current.getUser().getLogin());
    ...
    public UserRole getUserRole(entity.UserRolePK id) {
    ...
    public static class UserRoleControllerConverter implements Converter {
        ...
        entity.UserRolePK getKey(String value) {
            entity.UserRolePK key;
            String values[] = value.split(SEPARATOR_ESCAPED);
            key = new entity.UserRolePK();
            key.setRole(values[0]);
            key.setLogin(values[1]);
            return key;
        }

        String getStringKey(entity.UserRolePK value) {
            StringBuilder sb = new StringBuilder();
            sb.append(value.getRole());
            sb.append(SEPARATOR);
            sb.append(value.getLogin());
            return sb.toString();
        }
